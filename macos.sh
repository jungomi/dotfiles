...
# Disable all automatic typing help (auto correct capitalise, smart quotes etc.)
...
# Jump to the position when clicking on a scroll bar
...
# Show icons for servers, and removable media on the desktop but not hard drives.
...
# Enable spring loading for directories with no delay (enterting the directory while hovering with an item)
